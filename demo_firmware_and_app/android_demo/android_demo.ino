...
/*
 * Modified version of accel.ino for demo.
 * 
 * This is the firmware you are expected to run with Android application.
 * 
 * Differnces are:
 * (1) 100 instead of 1000 for batch size
 * (2) & and % as delimiters telling us the batch boundaries
 * (3) 500ms instead of 1000ms of delay for communication.
 */
...
void setup()
{
  ...
  Serial.println("Turn on BLE UART");
  HWSERIAL.begin(9600);
...
void loop()
{
  ...
  this_wakeup = micros();
  dt = this_wakeup - last_wakeup;
  last_wakeup = this_wakeup;
  ...
  data[datai++] = accelCount[0];
  data[datai++] = accelCount[1];
  data[datai++] = accelCount[2];
  
  if (datai >= BATCH_SIZE*3) {
      //Delimiter for start of batch data
      Serial.print("& ");
      HWSERIAL.print("& ");
      for (int i = 0; i < BATCH_SIZE*3; i++) {
          Serial.print(data[i]);
          Serial.print(" ");
          HWSERIAL.print(data[i]);
          HWSERIAL.print(" ");
      }
      //Delimiter for end of batch data
      Serial.println("%");
      HWSERIAL.println("%");
      datai = 0;
      delay(500);
