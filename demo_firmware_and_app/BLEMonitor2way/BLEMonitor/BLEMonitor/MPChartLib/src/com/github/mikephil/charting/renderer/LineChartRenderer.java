    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = 0; j < range; j += 4) {
                ...
                mBgPaint.setColor(dataSet.getColor(j / 4 + minx) & 0x3FFFFFFF);
                canvas.drawRect(buffer.buffer[j], mViewPortHandler.contentTop(), buffer.buffer[j + 2], mViewPortHandler.contentBottom(), mBgPaint);
