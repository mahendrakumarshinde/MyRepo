...
{
    public:
        static HardwareSerial *BLEport = &Serial1;
    ...
    private:
        enum {m_bufferLength = 19};
        char m_buffer[m_bufferLength];  // BLE Data buffer
        uint8_t m_bufferIndex; // BLE buffer Index
};

IUBLE::IUBLE() : m_buffer("abcdefghijklmnopqr"), m_bufferIndex(0)
{
  //constructor
}

/**
 * @brief prints given buffer then flushes
...
/*
  // This is currently in setup => make a function
...
{
  port->print("REC,");
  port->print(MAC_ADDRESS);
  if (rec_axis == 'X')
  {
    port->print(",X");
    for (int i = 0; i < MAX_INTERVAL_ACCEL; i++) {
      port->print(",");
      port->print(LSB_to_ms2(accel_x_batch[buffer_compute_index][i]));
      port->flush();
    }
  }
  else if (rec_axis == 'Y')
  {
    ...
    for (int i = 0; i < MAX_INTERVAL_ACCEL; i++) {
      port->print(",");
      port->print(LSB_to_ms2(accel_y_batch[buffer_compute_index][i]));
      port->flush();
    }
  }
  else if (rec_axis == 'Z')
  {
    ...
    for (int i = 0; i < MAX_INTERVAL_ACCEL; i++) {
      port->print(",");
      port->print(LSB_to_ms2(accel_z_batch[buffer_compute_index][i]));
      port->flush();
    }
  }
  port->print(";");
