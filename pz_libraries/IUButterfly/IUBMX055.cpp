...
IUBMX055::IUBMX055(IUI2C iuI2C, IUBLE iuBLE) : IUABCProducer(), m_iuI2C(iuI2C), m_iuBLE(iuBLE)
{
  for (int i=0; i < 3; i++)
  {
    m_rawAccel[i] = 0;
...
{
  byte* axb = (byte *) &m_accelData[0];
  byte* ayb = (byte *) &m_accelData[1];
  byte* azb = (byte *) &m_accelData[2];
