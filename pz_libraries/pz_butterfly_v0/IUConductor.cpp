...
IUConductor::IUConductor(String macAddress) :
  ...
  m_opState(operationState::notCutting),
 ...
 *
 */
void IUConductor::updateSensorConfiguration(IUSensorConfigurator::sensorConfig sensorConf)
{
 ...
 *
 */
void IUConductor::updateFeatureConfiguration(String configBuffer)
{
  featureConfigurator.requireConfiguration(configBuffer);
}
...
{
  // Monitor mode change ()
  if (m_opMode != operationMode::sleep && mode == operationMode::sleep)
  {
    m_sleepStartTime = millis();
  }
  m_opMode = mode;
  String msg;
  IURGBLed *led = sensorConfigurator.iuRGBLed;
  ...
  {
    beginDataAcquisition();
    led->changeColor(IURGBLed::CYAN_DATA);
    featureConfigurator.resetFeaturesCounters();
    msg = "data collection";
  }
 ...
 * NB1: Also updates the notCuttingStartTime timer.
 * NB2: When in sleep mode, the led color doesn't change
 */
void IUConductor::switchToState(operationState state)
{
  if (m_opState != operationState::notCutting && state == operationState::notCutting)
  {
    m_notCuttingStartTime = millis(); // Update m_notCuttingStartTime if needed
  }
  m_opState = state;
  if (m_opMode == operationMode::sleep)
  {
    return; // If sleeping, do not change led color
  }
  IURGBLed *led = sensorConfigurator.iuRGBLed;
  if (m_opState == operationState::notCutting)
  {
    led->changeColor(IURGBLed::BLUE_NOOP);
  }
  else if (m_opState == operationState::normalCutting)
  {
    led->changeColor(IURGBLed::GREEN_OK);
  }
  else if (m_opState == operationState::warningCutting)
  {
    led->changeColor(IURGBLed::ORANGE_WARNING);
  }
  else if (m_opState == operationState::badCutting)
  {
    led->changeColor(IURGBLed::RED_BAD);
  }
}
...
bool IUConductor::checkAndUpdateMode()
{
  ...
  {
    if (m_opMode != operationMode::sleep && m_startSleepTimer < millis() - m_notCuttingStartTime)
    {
      switchToMode(operationMode::sleep); // go to sleep
      return true;
    }
    else if (m_opMode == operationMode::sleep && m_opState == operationState::notCutting && m_endSleepTimer < millis() - m_sleepStartTime)
    {
...
{
  // Acquire data only if all ok and data acquisition must have been started
  if (iuI2C->getErrorMessage().equals("ALL_OK") && m_inDataAcquistion)
  {
    if (m_opMode == operationMode::run)
    {
