...
public abstract class AbstractChart implements Serializable {
  ...
  protected int drawLegend(Canvas canvas, DefaultRenderer renderer, String[] titles, int left,
    ...
    //float size = 32;
    float size = RoundChart.SHAPE_WIDTH;
    if (renderer.isShowLegend()) {
      ...
      for (int i = 0; i < sLength; i++) {
        ...
        if (r.isShowLegendItem()) {
          ...
          float extraSize = lineSize + renderer.getLegendTextSize() / 2 + sum;
          ...
          if (!calculate) {
            ...
            drawString(canvas, text, currentX + lineSize + 5, currentY + RoundChart.SHAPE_WIDTH / 2, paint);
          }
          currentX += extraSize;
        }
      }
