...
uint8_t splitString(String str, const char separator, String destination[], const uint8_t destinationSize)
{
  ...
  for (int i = 0; i < destinationSize; i++)
  {
    ...
    if (idx == -1)
    {
      destination[i] = str.substring(startAt);
      for (int j = i + 1; j < destinationSize; j++)
      {
        destination[j] = "";
      }
      return i + 1;
...
float computeAudioRFFT(uint8_t sourceCount, const uint16_t* sourceSize, q15_t* source[])
{
  ...
  uint16_t ss = sourceSize[0];
  const uint16_t rfftSize = ss / 2;
  q15_t compBuffer[ss];
  float magSize(0), hammingK(0), inverseWLen(0);
  switch(rfftSize)
  {
    case 512:
      arm_mult_q15(source[0], hamming_window_512, compBuffer, rfftSize);
      arm_mult_q15(&source[0][rfftSize], hamming_window_512, &compBuffer[rfftSize], rfftSize);
      magSize = magsize_512;
      hammingK = hamming_K_512;
      inverseWLen = inverse_wlen_512;
      break;
    case 2048:
      arm_mult_q15(source[0], hamming_window_2048, compBuffer, rfftSize);
      arm_mult_q15(&source[0][rfftSize], hamming_window_2048, &compBuffer[rfftSize], rfftSize);
  ...
  for (int i = 0; i < 2; i++)
  {
    arm_status status = arm_rfft_init_q15(&rfftInstance,  // RFFT instance
                                          ...
                                          0,              // 0: forward FFT, 1: inverse FFT
                                          1);             // 0: disable bit reversal output, 1: enable it
    arm_rfft_q15(&rfftInstance, &source[0][i * rfftSize], rfftBuffer);
    arm_shift_q15(&rfftBuffer[i * rfftSize], AUDIO_RESCALE, &rfftBuffer[i * rfftSize], ss);
  }

  // NOTE: OUTPUT FORMAT IS IN 2.14
  arm_cmplx_mag_q15(rfftBuffer, source[0], ss);
  arm_q15_to_float(source[0], (float*)rfftBuffer, magSize);

  // TODO: REDUCE VECTOR CALCULATION BY COMING UP WITH SINGLE FACTOR
  // Div by K
  arm_scale_f32((float*)rfftBuffer, hammingK, (float*)rfftBuffer, magSize);
  // Div by wlen
  arm_scale_f32((float*)rfftBuffer, inverseWLen, (float*)rfftBuffer, magSize);
  // Fix 2.14 format from cmplx_mag
  arm_scale_f32((float*)rfftBuffer, 2.0, (float*)rfftBuffer, magSize);
  // Correction of the DC & Nyquist component, including Nyquist point.
  arm_scale_f32(&((float*)rfftBuffer)[1], 2.0, &((float*)rfftBuffer)[1], magSize - 2);
  return 0;
}
