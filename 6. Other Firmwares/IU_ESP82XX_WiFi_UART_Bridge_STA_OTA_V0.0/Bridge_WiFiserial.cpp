...
void WiFiSerial::evaluateCommand()
{
  switch(cmdWiFiSerial[CURRENTPORT])
  {
   case RECEIVE_SERIAL_DATA:                                                                // CMD 10
     ...
     break;
   case GO_TO_SLEEP:                                                                        // CMD 11
     float sleepTimeS;
     sleepTimeS = WiFiSerial::readfloat();
     ESP.deepSleep((uint64_t)(sleepTimeS*1000000.0f), RF_DEFAULT);
     delay(100000);
     break;
   case SEND_SERIAL_DATA:                                                                   // CMD 20
     ...
     break;
   case XMIT_ACK_NAK_BYTE:                                                                  // CMD 22
     ack_nak_byte = WiFiSerial::read8();
     WiFi_UDP::XMIT_OTA_ACK_NAK();
     break;
   default:
     break;
  }
...
/**
* @fn:  WiFiSerial::WiFi_Bridge_Ready(uint8_t BridgeStatByte)
*
* @brief: Tells the host that the bridge is connected to a valid host and is ready to receive data
* @params:
* @returns:
*/
void WiFiSerial::WiFi_Bridge_Ready(uint8_t BridgeStatByte)
{
  CURRENTPORT = 0;
  cmdWiFiSerial[0] = 21;                                                                    // Call CMD 21, "WIFI_BRIDGE_READY" payload 1 byte
  WiFiSerial::headSerialRequest(0, 0, 1);
  WiFiSerial::serialize8(BridgeStatByte);
  WiFiSerial::tailSerialRequest(0);
}

/**
* @fn:  WiFiSerial::Initiate_OTA_Update()
*
* @brief: Sends the new image size and number of data blocks to the hust to initiate OTA FW update
* @params:
* @returns:
*/
void WiFiSerial::Initiate_OTA_Update()
{
  CURRENTPORT = 0;
  cmdWiFiSerial[0] = 12;                                                                    // Call CMD 12, "INITIATE_OTA_UPDATE" payload 6 bytes
  WiFiSerial::headSerialRequest(0, 0, 6);
  for(uint8_t i=0; i<6; i++)
  {
    WiFiSerial::serialize8(OTA_bytes[i]);
  }
  WiFiSerial::tailSerialRequest(0);
}

/**
* @fn:  WiFiSerial::Push_OTA_Data()
*
* @brief: Pushes the current values of OTA update array into a serial command; data variables treated as unsigned bytes
* @params:
* @returns:
*/
void WiFiSerial::Push_OTA_Data()
{
  CURRENTPORT = 0;
  cmdWiFiSerial[0] = 13;                                                                    // Call CMD 13, "RECEIVE_OTA_DATA" payload 130 bytes
  WiFiSerial::headSerialRequest(0, 0, 130);
  for(uint8_t i=0; i<130; i++)                                                              // First two bytes of the array are the block number in uint16_t form
  {
    WiFiSerial::serialize8(OTA_bytes[i]);
  }
  WiFiSerial::tailSerialRequest(0);
