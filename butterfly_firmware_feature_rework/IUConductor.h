...
/**
 * A class to hold, create, configure and activate features
 *
...
{
    public:
        // Hz (must be compatible with I2C)
        static const uint16_t defaultClockRate = 48000;
        // ms - send data every T milliseconds
        static const uint16_t defaultDataSendPeriod = 512;
        // Default start datetime
        static constexpr double defaultTimestamp = 1492144654.00;
        // ms - send data every T milliseconds
        static const uint16_t shortestDataSendPeriod = 512;
        ...
        void configureAutoSleep(bool enabled, uint16_t startSleepTimer,
                                uint32_t sleepDuration);
        void manageSuspendCycle();
        void configureSuspendCycle(bool enabled, uint32_t onTime,
                                   uint32_t cycleTime);
