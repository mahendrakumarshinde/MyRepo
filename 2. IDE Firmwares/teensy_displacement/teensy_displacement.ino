...
// Target frequencies may change in data collection mode
uint16_t TARGET_AUDIO_SAMPLE = AUDIO_FREQ;
uint16_t TARGET_ACCEL_SAMPLE = ACCEL_FREQ;
...
//TODO: May be able to use just one instance.
arm_rfft_instance_q15 audio_rfft_instance;
arm_rfft_instance_q15 accel_rfft_instance;


// New features
float featureAccelRMSX(0), featureAccelRMSY(0), featureAccelRMSZ(0);
float featureMainFreqX(0), featureMainFreqY(0), featureMainFreqZ(0);
float featureVelX(0), featureVelY(0), featureVelZ(0);
float featureDispX(0), featureDispY(0), featureDispZ(0);
...
const int defaultDatasendlimit = 1500;
int datasendlimit = defaultDatasendlimit;
int currentmillis = 0;
int prevmillis = 0;
int currenttime = 0;
int prevtime = 0;
...
// more states.
float featureNormalThreshold[NUM_FEATURES] = {100,   // Feature Energy
                                              0.05, // Velocity X
                                              0.05, // Velocity Y
                                              0.05, // Velocity Z
                                              200,  // Temperature
...
float featureWarningThreshold[NUM_FEATURES] = {115, // Feature Energy
                                               1.2, // Velocity X
                                               1.2, // Velocity Y
                                               1.2, // Velocity Z
                                               205, // Temperature
...
float featureDangerThreshold[NUM_FEATURES] = {140, // Feature Energy
                                              1.8,  // Velocity X
                                              1.8,  // Velocity Y
                                              1.8,  // Velocity Z
                                              210,  // Temperature
 ...
 * 
 */
float press_energy(uint16_t startInd, uint32_t buffSize, bool removeMean=false)
{
  ...
  for (uint16_t i=0; i < 3; i++)
  {
    ...
    if (removeMean)
    {
      arm_mean_f32(compBuffer[i], buffSize, &avg);
    }
    // Remove accel biases
    // arm_offset_f32(compBuffer[i], accelBias[i], compBuffer[i], buffSize);
    for (uint16_t j = 0; j < buffSize; j++)
    {
      energy[i] += sq(compBuffer[i][j] - avg);
    }
    energy[i] /= buffSize;
  }
  featureAccelRMSX = energy[0];
  featureAccelRMSY = energy[1];
  featureAccelRMSZ = energy[2];
  if (featureDebugMode)
  {
    Serial.print("Accel RMS: ");
    Serial.print(featureAccelRMSX);
    Serial.print(", ");
    Serial.print(featureAccelRMSY);
    Serial.print(", ");
    Serial.println(featureAccelRMSZ);
  }
...
void getVelocityAndDisplacement(q15_t *values, uint16_t samplingRate,
                                uint16_t sampleCount, uint16_t lowCutFrequency,
                                uint16_t highCutFrequency,
                                float &mainFrequency, float &velocity,
                                float &displacement)
{
  // 0. Preparation
  // float resolution = 1; // FIXME
  ...
  velocity = aRes * 9.8065 * integratedRMS1 * 1000.0 / ((float) scaling1);
  if (featureDebugMode)
  {
  ...
  displacement = aRes * 9.8065 * integratedRMS2 * 1000.0 / ((float) scaling1 * (float) scaling2);
  if (featureDebugMode)
  {
...
void compute_features() {
  ...
  LEDColors c;
  if (!recordmode) {
    if (sleepmode && highest_danger_level == 0) {
      // Do nothing
    }
    else if (highest_danger_level != currState || datasendtime)
    {
      currState = (OpState) highest_danger_level;
      if (currMode == RUN)
      {
          BLEport.print(MAC_ADDRESS);
          BLEport.print(",0");
          BLEport.print(currState);
          BLEport.print(",");
          BLEport.print(bat);
      ...
      else if (currMode == CALIBRATION)
      {
          ...
          Serial.print(bat);
          Serial.print(",0001,");
          Serial.print(featureVelX);
          Serial.print(",0002,");
          Serial.print(featureVelY);
          Serial.print(",0003,");
          Serial.print(featureVelZ);
          Serial.print(",0004,");
          Serial.print(feature_value[4]);  // Temperature
          Serial.print(",0005,");
          Serial.print(featureMainFreqX);
          Serial.print(",0006,");
          Serial.print(featureMainFreqY);
          Serial.print(",0007,");
          Serial.print(featureMainFreqZ);
          Serial.print(",0008,");
          Serial.print(featureAccelRMSX);
          Serial.print(",0009,");
          Serial.print(featureAccelRMSY);
          Serial.print(",0010,");
          Serial.print(featureAccelRMSZ);
          Serial.print(",");
...
void i2s_rx_callback( int32_t *pBuf )
{
  ...
  if (error_message.equals("ALL_OK"))
  {
    ...
    if (currMode == RUN || currMode == CALIBRATION)
    {
...
/* =============================================================================
  Component and hardware related functions
============================================================================= */
...
void loop()
{
  ...
  if (wireBuffer.length() > 0)
  {
    if (currMode == RUN)
    {
      ...
      else if (wireBuffer.indexOf(START_CALIBRATION) > -1) {
        LEDColors c = CYAN_DATA;
        changeStatusLED(c);
        Serial.print(START_CONFIRM);
        resetSampling(false);
        currMode = CALIBRATION;
        datasendlimit = 500;
      }
    }
    else if (currMode == DATA_COLLECTION)
    ...
    else if (currMode == CALIBRATION)
    {
      if (wireBuffer == END_CALIBRATION)
      {
        ...
        accel_counter = 0;
        subsample_counter = 0;
        datasendlimit = defaultDatasendlimit;
      }
    }
  ...
  if ((currMode == RUN || currMode == CALIBRATION) && compute_feature_now[buffer_compute_index] && error_message.equals("ALL_OK")) {
    compute_features();
  }
}
