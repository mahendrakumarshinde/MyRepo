...
float press_energy(uint16_t startInd, uint32_t buffSize)
{
  ...
  for (uint16_t i=0; i < 3; i++)
  {
    ...
    {
      energyByAxis[i] += sq(compBuffer[i][j]);
    }
    energyByAxis[i] /= buffSize;
  }
  return (energyByAxis[0] + energyByAxis[1] + energyByAxis[2]); //Total energy on all 3 axes
}
