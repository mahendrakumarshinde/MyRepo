...
uint32_t I2S_CLOCK_RATE = 48000;                        // Hz
bool statusLED = true;                                  // Status LED ON/OFF
String MAC_ADDRESS = "88:4A:EA:69:38:22";

bool MEASURE_CALLBACK = false;
bool accelDataReadyFlag = false;
...
uint32_t DOWNSAMPLE_MAX = I2S_CLOCK_RATE / TARGET_AUDIO_SAMPLE;  // Down-sampling ratio
...
{
  uint32_t cbStartTime = 0;
  if (MEASURE_CALLBACK && currMode == DATA_COLLECTION)
  {
    cbStartTime = micros();
  }
  //  Serial.print("I2S RX : ");
  ...
  if (error_message.equals("ALL_OK"))
  {
    ...
    if (subsample_counter < DOWNSAMPLE_MAX - 1)
    {
      subsample_counter ++;
      return;
    } else {
...
//        readAccelData(accelCount);
  ...
  if (MEASURE_CALLBACK && currMode == DATA_COLLECTION)
  {
    uint32_t dt = micros() - cbStartTime;
    Serial.print("dt: ");
    Serial.println(dt);
  }
}
  ...
  if (wireBuffer.length() > 0)
  {
    ...
    else if (currMode == DATA_COLLECTION)
    {
      ...
      if (wireBuffer.indexOf("acosr") > -1)
      {
        ...
        DOWNSAMPLE_MAX = I2S_CLOCK_RATE / TARGET_AUDIO_SAMPLE;
