...
bool UpdateClass::_verifyEnd() {
    ...
    if(_command == U_FLASH) {
        if(!_enablePartition(_partition) || !_partitionIsBootable(_partition)) {
            Serial.println("Enable/Boot Error");
            _abort(UPDATE_ERROR_READ);
            return false;
        }
        boot_err = esp_ota_set_boot_partition(_partition);
        if(boot_err){
            Serial.print("Set Boot Error");
            Serial.println(boot_err);
...
bool UpdateClass::end(bool evenIfRemaining){
    ...
    return true; //_verifyEnd();  _MSS
}

bool UpdateClass::end1(bool evenIfRemaining){
    return _verifyEnd();
}
...
size_t UpdateClass::writeStream_OTA(Stream &data) {
    size_t written = 0;
    size_t toRead = 0;
    int loopCnt = 0;
    if(hasError() || !isRunning())
...
size_t UpdateClass::writeStream_SPIFFS(String fwpath, long FileLength) {
    size_t written = 0;
    size_t toRead = 0;
    int loopCnt = 0;
    char loopCnt1 = 0;
    ...
    Serial.println("Opening bin:" + String(fwpath));

    file = SPIFFS.open(fwpath.c_str(), FILE_READ);
    if(!file){
        Serial.println("- failed to open file for reading");
        return 0;
    }
    Serial.print("Next Partition Add:0x");
    Serial.print(_partition->address,HEX);
    Serial.print(" Type:");
    Serial.print(_partition->type,DEC);
    Serial.print(" Subtype:");
    Serial.print(_partition->subtype,DEC);
    Serial.print(" Size:0x");
    Serial.println(_partition->size,HEX);
   ...
   //     size_t flen = len;
    ...
    //      totalblock = totalblock + 1;
        ...
        while(len)
        {
            ...
            writelen = writelen + toRead;
            delay(10);
//            Serial.print("Block:");
//            Serial.print(toRead,DEC);
            file.read(&_buffer[loopCnt1*512], toRead);
            loopCnt1++;
            if((loopCnt+1) % 8 == 0 || toRead < 512)
            {
                //loopCnt++;
                if(!ESP.flashEraseSector((_partition->address + progress)/SPI_FLASH_SEC_SIZE)){                    
                    return false;
                }
                if (!ESP.flashWrite(_partition->address + progress, (uint32_t*)_buffer, writelen)) {
                   return false;
                }
     //           Serial.println("Flash Writing Sector: " + String(loopCnt) + "/" + String(totalblock));
                Serial.print("FlashWrite Add:0x");
                Serial.print(_partition->address+progress,HEX);
                progress = progress + writelen;
  //              Serial.print("Loop:");
  //              Serial.print(loopCnt);
