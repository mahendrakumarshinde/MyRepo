...
// Modified for custom data collection from I2S instead
...
// allocate  data buffer... This buffer may not be used for now.
...
unsigned int soundTimeCount;
unsigned int accelTimeCount;
...
void i2s_rx_callback( int32_t *pBuf )
{
  ...
  totalTime += currTime - prevTimeMark;
  soundTimeCount ++;
  prevTimeMark = currTime;
  
  // perform the data extraction for both channel sides
  // Most of this data will be thrown out, but collect for simulation sake.
  ...
  if (accel_counter == 8){
    ...
    /*
     *  Ignore gyro and mag data for now.
     * 
  ...
  if (soundTimeCount == target_sample){
    // Print statistics every second.
    float soundInterTime = (float)totalTime/(float)soundTimeCount;
    float accelInterTime = (float)totalTime/(float)accelTimeCount;
    ...
    Serial2.printf("\n\nTesting Statistics:\n");
    Serial2.printf("Inter-sample period for Sound: %f microseconds\n", soundInterTime);
    Serial2.printf("Inter-sample period for Accel: %f microseconds\n", accelInterTime);

    Serial2.print("Last IMU data stamp:\n");
    ...
    /*
    Serial2.print("gx = "); Serial2.print( gx, 2); 
    Serial2.print("  gy = "); Serial2.print( gy, 2); 
    Serial2.print("  gz = "); Serial2.print( gz, 2); Serial2.println(" deg/s");
    Serial2.print("mx = "); Serial2.print( (int)mx); 
    Serial2.print("  my = "); Serial2.print( (int)my); 
    Serial2.print("  mz = "); Serial2.print( (int)mz); Serial2.println(" mG");
    */

    Serial2.print("Last sound data stamp:\n");
    Serial2.print(pBuf[0]);
    Serial2.print("\n");
  }
}
...
void loop()
{  
  // Don't do anything in the loop function. Do everything based on the I2S timer.
