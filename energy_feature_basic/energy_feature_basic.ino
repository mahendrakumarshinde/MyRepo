...
Infinite Uptime Energy Feature Basic Firmware
...
const uint16_t target_audio_sample = 4000;
const uint16_t target_accel_sample = 1000;
...
const uint32_t energy_interval = 100;
uint32_t samplingCount = 0;
float accel_x[energy_interval];
float accel_y[energy_interval];
float accel_z[energy_interval];
float threshold = 100;
float ave_x = 0;
float ave_y = 0;
float ave_z = 0;
...
const uint32_t resting_interval = 0;
...
void i2s_rx_callback( int32_t *pBuf )
{
  ...
  if (accel_counter == accel_counter_target){
    ...
    if (restCount < resting_interval){
      ...
      accel_x[samplingCount] = ax;
      accel_y[samplingCount] = ay;
      accel_z[samplingCount] = az;
      samplingCount ++;
      if (samplingCount == energy_interval){
        // Calculate energy
        ave_x=0;
        ave_y=0;
        ave_z=0;
        for (int i=0; i<energy_interval; i++){
          ave_x += accel_x[i];
          ave_y += accel_y[i];
          ave_z += accel_z[i];
        }
        ave_x /= energy_interval;
        ave_y /= energy_interval;
        ave_z /= energy_interval;

        ene = 0; //initialize the energy feature
        for (int i=0; i<energy_interval; i++){
          ene += sq(accel_x[i]-ave_x) + sq(accel_y[i]-ave_y) + sq(accel_z[i]-ave_z);
        }
        ene *= energy_interval;
...
void loop()
{
  if(Serial2.available() > 0){
    ...
    Serial2.readBytes(commandBuffer, Serial2.available());
    Serial.print("Received from HM11: ");
    Serial.println(commandBuffer);
  }
  if(Serial.available() > 0){
    memset(&commandBuffer[0], 0, sizeof(commandBuffer));
    Serial.readBytes(commandBuffer, Serial.available());
