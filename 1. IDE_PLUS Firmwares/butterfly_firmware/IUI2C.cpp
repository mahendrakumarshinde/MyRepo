 ...
 * Write a byte to given address and sub-address
 */
bool IUI2C1::writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
    uint8_t temp[2];
    temp[0] = subAddress;
    temp[1] = data;
    byte errorCode = Wire1.transfer(address, &temp[0], 2, NULL, 0);
    if (errorCode != 0x00)
    {
        m_errorFlag = true;
        if (debugMode)
                      ...
                      void(*callback)(uint8_t wireStatus))
{
    ...
    temp[0] = subAddress;
    temp[1] = data;
    bool success = Wire1.transfer(address, &temp[0], 2, NULL, 0, true, callback);
    if (!success)
    {
        if (asyncDebugMode)
        {
...
bool IUI2C1::writeWord(uint8_t address, uint8_t subAddress, uint16_t data)
{ 
    uint8_t temp[3];
    temp[0] = subAddress;
    temp[1] = (byte) ( data >> 8 ) & 0xFF;
    ...
    byte errorCode = Wire1.transfer(address, &temp[0], 3, NULL, 0);
    if (errorCode != 0x00)
    {
        if (asyncDebugMode)
        {
            debugPrint(F("I2C1 Temperature Limit set error"));
        }
        return false;
    }
    return true;
 ...
 * @param subAddress Where to read the byte from
 */
uint8_t IUI2C1::readByte(uint8_t address, uint8_t subAddress)
{
    uint8_t temp[1];
    byte errorCode = Wire1.transfer(address, &subAddress, 1, &temp[0], 1);
    if (isError())
    {
        m_errorFlag = true;
        if (debugMode)
...
{
    uint8_t temp[1];
    if (!Wire1.transfer(address, &subAddress, 1, &temp[0], 1, true, callback))
    {
        if (asyncDebugMode)
        {
            debugPrint(F("I2C error"));
 ...
 * @param The number of byte to read
 */
bool IUI2C1::readBytes(uint8_t address, uint8_t subAddress, uint8_t count,
                      uint8_t *destination)
{
    if (!m_readFlag)
    {
    ...
    m_readFlag = false;
    byte errorCode = Wire1.transfer(address, &subAddress, 1, destination, count);
    m_readFlag = true;
    if (errorCode != 0x00)
    {
        m_errorFlag = true;
                      ...
                      uint8_t *destination, void(*callback)(uint8_t wireStatus))
{
    ...
    m_readFlag = false;
    bool success = Wire1.transfer(address, &subAddress, 1, destination, count,
                                 true, callback);
    if (!success)
    {
        if (asyncDebugMode)
...
* @return False if no device found else true
*/
bool IUI2C1::scanDevices()
{
    if (setupDebugMode)
    {
        debugPrint(F("I2C1 Scanning..."));
    }
    byte error, address;
    int nDevices = 0;
    for (address = 1; address < 127; address++ )
 ...
 * @return True if the WHO_AM_I register read is correct, else false
 */
bool IUI2C1::checkComponentWhoAmI(String componentName, uint8_t address,
                                 uint8_t whoAmI, uint8_t iShouldBe)
{
    byte c = readByte(address, whoAmI);  // Read WHO_AM_I register for component
    if (setupDebugMode)
