...
{
  read_all_flags();
  all_flags[MFW_FLASH_FLAG] = 1; /* Forcing the Flag, for testing */
  
//switch (all_flags[MFW_FLASH_FLAG])
  switch(MFW_FLAG)
  {
    ...
    case 1:  /* 1 -> Flash STM Main Firmware */
          ...
          /*Verify_STM_FW();
           * If Error, all_flags[MFW_FLASH_FLAG] = 2; else all_flags[MFW_FLASH_FLAG] =3;
           */
          break;
    ...
    case 4: /* 4-> Rollback STM MFW*/
           ...
           */
           rename("STM-MFW1.bin", "STM-TEMP.bin");
           delay(1000);
           rename("STM-MFW2.bin", "STM-MFW1.bin");
           delay(1000);
           rename("STM-TEMP.bin", "STM-MFW2.bin");
           delay(1000);
           remove("STM-TEMP.bin");
           delay(1000);
           Flash_STM_File(STM_MFW_1);
...
void read_all_flags()
{
  ...
  for(int i = 0 ; i <128 ; i++)
  {
    ...
    for (int i = 0 ; i < 128; i= i+8)
        all_flags[i] = *(volatile uint8_t*)(FLAG_ADDRESS + i);
...
//  for (int i=1; i <= (dataFileSize/TEST_CHUNK_SIZE+1); i++) {
  ...
  {
    DEBUG_SERIAL.println("Flashing completed updating flag..."); 
...
//        FlashData[i] = 0xAA;
   ...
   uint8_t flag_addr_temp = flag_addr * 8;
    read_all_flags();
    all_flags[flag_addr_temp] = flag_data;
    
//    all_flags[flag_addr] = read_flag(flag_data);

    
    stm32l4_flash_erase((uint32_t)FLAG_ADDRESS,2048);
    delay(1000);
    DEBUG_SERIAL.println("Flash Erased...");
    stm32l4_flash_program((uint32_t)(FLAG_ADDRESS),&all_flags[0],128);
    delay(1000);
