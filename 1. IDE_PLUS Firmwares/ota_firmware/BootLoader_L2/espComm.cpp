...
bool espComm::flash_esp32_verify()
{
 // DOSFS.begin();
  ...
  bool ret = esp_SendSyncCmd(10,10);
  if(ret)  
  {
    ...
    espBinWrite("iuTempFirmware","wifi_ESP32.ino.bin");
    espFlashLog.close();
    delay(100);
    espFlashLog = DOSFS.open("esp32Response.log", "a");
    delay(100);
        ...
        if(fwFile)
        {
            ...
            if(fileSize > 0)
            {
              ...
              // Set File size in Flash configure- Address set command 
                espSendCmd(FlashAdd, sizeof(FlashAdd), 1);
                delay(100);
                do
                {                    
                    if(fileSize >= FLASH_BLOCK_SIZE)
                    {
                       ...
                       // DEBUG_SERIAL.println(readIdx);
                        ...
                        preparePkt(readBuf,pktSeqNo,readIdx);
                        pktSeqNo++;
                    ...
                    {     
                      ...
                      //  DEBUG_SERIAL.print(F("File Read Size:"));
                       // DEBUG_SERIAL.println(readIdx);
                        ...
                        preparePkt(readBuf,pktSeqNo,FLASH_BLOCK_SIZE);        
                    }                        
                } while (fileSize);
                fwFile.close();
                return true;                
...
{    
  ...
  //  DEBUG_SERIAL.println("Modified packet size:" + String(pktSize1));
   ...
   // DEBUG_SERIAL.println("Total packet size:" + String(writeIdx));
    ...
    //  }
    // }
   espSendDataPkt(PktBuf, writeIdx, 1,pktSeqNo);
   delay(100);
}
...
bool espComm::espDetect()
{
    ...
    espSendCmd(deviceType, sizeof(deviceType), 1);
    delay(100);

    espSendCmd(chipId, sizeof(chipId), 2);  
    delay(100);
}
...
bool espComm::espGetFeature()
{
    ...
    espSendCmd(readReg1, sizeof(readReg1), 1);
    delay(100);

    espSendCmd(readReg2, sizeof(readReg2), 1);
    delay(100);
}
...
bool espComm::espGetMacId()
{
    ...
    espSendCmd(MacIDreg1, sizeof(MacIDreg1), 1);
    delay(100);

    espSendCmd(MacIDreg2, sizeof(MacIDreg2), 1);
    delay(100);
}
...
bool espComm::espUploadStub()
{
    ...
    espSendCmd(uploadStub, sizeof(uploadStub), 1);
    delay(100);

    espSendCmd(uploadStub0, sizeof(uploadStub0), 1);
    delay(100);

    espSendCmd(uploadStub1, sizeof(uploadStub1), 1);
    delay(100);

    espSendCmd(uploadStub2, sizeof(uploadStub2), 1);
    delay(100);

    espSendCmd(uploadStub3, sizeof(uploadStub3), 1,4);
    delay(100);
}
...
bool espComm::espConfigureFlash()
{
    ...
    espSendCmd(configFlash1, sizeof(configFlash1), 1);
    delay(100);
    espSendCmd(configFlash2, sizeof(configFlash2), 1);
    delay(100);
    espSendCmd(configFlash3, sizeof(configFlash3), 1);
    delay(100);
    espSendCmd(configFlash4, sizeof(configFlash4), 1);
    delay(100);
    espSendCmd(configFlash5, sizeof(configFlash5), 1);
    delay(100);
    espSendCmd(configFlash6, sizeof(configFlash6), 1);
    delay(100);
    espSendCmd(configFlash7, sizeof(configFlash7), 1);
    delay(100);
    espSendCmd(configFlash8, sizeof(configFlash8), 1);
    delay(100);
    espSendCmd(configFlash9, sizeof(configFlash9), 1);
    delay(100);
    espSendCmd(configFlash10, sizeof(configFlash10), 1);
    delay(100);
    espSendCmd(configFlash11, sizeof(configFlash11), 1);
    delay(100);
    espSendCmd(configFlash12, sizeof(configFlash12), 1);
    delay(100);
    // espSendCmd(FlashAdd, sizeof(FlashAdd), 1);
    // delay(100);
}
...
{
  espSendCmd(FlashLeave1, sizeof(FlashLeave1), 1);
  delay(100);
  espSendCmd(FlashLeave2, sizeof(FlashLeave2), 1);
  delay(100);
}
...
#if 0
bool espComm:: send_sync_byte(int retry)
{
  int cnt = 32;
  DEBUG_SERIAL.println(sizeof(syncCommand));
  espFlashLog.print("Sync Command : ");
...
*/
bool espComm::send_command_with_response_c0(byte command[], int size, int retry)
{
  int cnt = 2;
  DEBUG_SERIAL.println(size);
  espFlashLog.print("Command  : ");
...
*/
bool espComm::send_command_with_response_c0_1(unsigned char *command, uint32_t size, int retry,uint16_t seq)
{
    int cnt = 2;
    DEBUG_SERIAL.println(size);
    espFlashLog.print("Data Packet, Seq:");
